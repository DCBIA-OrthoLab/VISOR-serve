...
# What a reader may do at a checkpoint, and therefore whether a later stop can
# send them BACK to it. The vocabulary is the tools', published in their
# schemas; this server names the default and nothing else.
REVIEW_VIEW = "view"

def _declared_checkpoints(declared):
    """`({name: kind}, (name, ...))` from what a tool published.

    Two shapes are accepted, because this is the seam between two
    repositories and a schema generated by an older `describe.py` must not
    stop this server from starting:

        ["landmarks"]                            the first shape
        [{"name": "landmarks", "kind": "view"}]  what it says now

    The kind says what a reader may DO at that checkpoint -- look at it, drag
    its points, drag the scan -- and therefore whether it is somewhere a
    reader can come BACK to from a later stop. Absent reads as `view`, which
    is the conservative direction: a checkpoint nobody declared as editable
    never offers a way back to it.
    """
    kinds = {}
    names = []
    for entry in declared or ():
        if isinstance(entry, dict):
            name = entry.get("name")
            kind = entry.get("kind") or REVIEW_VIEW
        else:
            name, kind = entry, REVIEW_VIEW
...
def _stop_after_spec(options, layout: dict, kinds=None) -> ArgSpec:
    ...
    return ArgSpec(
        ...
        ui=layout.get("ui") or "inline",
        option_help=layout.get("option_help"),
        # What a reader may do at each of them, so a panel can offer a
        # way BACK to the ones that can be edited and not to the ones
        # that can only be looked at.
        option_kind=dict(kinds) if kinds else None,
        visible_when=layout.get("visible_when"),
        hidden=bool(layout.get("hidden")),
    )
...
class SchemaTool(Tool):
    ...
    def __init__(self, folder: str, schema: dict, deployment: ToolDeployment):
        ...
        self.checkpoint_kinds, self.quality_controls = _declared_checkpoints(
            schema.get("quality_controls"))
        # What a reader may do with what THIS tool produced. It is how a stop
        # named after a CALL gets a kind at all: that boundary carries none of
    ...
    def publish_stops(self, points, kinds=None) -> None:
...
def load_tool(folder: str, config, name: str = None) -> SchemaTool:
    ...
    # Published back into the config, which is the only place anything but this
    # tool object can read what the conventions decided. The derived deployment
    # exists nowhere else: it is built here, handed to the `Tool`, and was
    # otherwise dropped -- so `execution/concurrency.width_axis`, which asks the
    # config for a tool's batch axis, found nothing for any tool that had not
    # spelled `width_from` out in deployment.toml.
    #
    # Recorded only once the tool has passed its own schema check, so a folder
    # that fails to load leaves nothing behind claiming to describe a tool this
    # server serves.
    config.record_resolved(name, deployment)
    return tool
